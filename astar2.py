...
class AStar(object):
    ...
    def update_node(self, nextNode, node):
        ...
        nextNode.g = self.heuristic(nextNode, self.start)#node.g + 10 #self.heuristic(nextNode, self.start)
        nextNode.h = self.heuristic(nextNode, self.end) #self.heuristic2(nextNode) #self.heuristic(nextNode, self.end)
            ...
            for neighbour in neighbours:
                if ~neighbour.obstacle and neighbour not in self.closed:
                    ...
                    # if neighbour is in open list, check if current path is better than the one previously found for this neighbour
                        if neighbour.g > node.g: # previously: > node.g + 10

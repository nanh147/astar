...
% This version loads up a previously generated map file

load('investigate-4.mat','goal_dist', 'MAP','gridsize')
...
% MAP(:,gridsize-1) = 2; % we should always have a path to the object
...
plot(x,y,'ko') % plot the target

[y_robot,x_robot] = find(MAP == 1); % locations of obstacles
plot(x_robot,y_robot,'rd') % plot the init
...
function index = nodeIndex(OPEN, xval, yval)
gh = 1;
...
function min_index = minFn(OPEN)
OPEN = [OPEN, [1:size(OPEN, 1)]']; % augment open with indices
temp = OPEN(OPEN(:,1) == 1, :);
if isempty(temp)
    min_index = -1;
else
    [~, min_index] = min(temp(:,8));
    min_index = temp(min_index, 9); % return index of OPEN list, not temp
end
end

function expanded = expandArray(x_node,y_node,hn,x_target,y_target,CLOSED,MAX_X,MAX_Y)
expanded = [];
exp_count = 1;
...
function dist = radistance(xcurr, ycurr, xdest, ydest)

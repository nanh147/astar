...
% A star with randomly placed circular obstacles
gridsize = 200;
obstacle_locs = round(rand(4)*gridsize);
obstacle_radius = 20;
...
% stick the obstacles in
for i = 1:size(obstacle_locs,1)
    pts = [];
    pts = gencircle2(obstacle_radius, obstacle_locs(i, 1),obstacle_locs(i, 2));
    idx = any(pts > MAX_X | pts <= 0,2); % remove inadmissable values (out of grid)
    pts(idx,:) = [];
    thing = sub2ind(size(obstacles),pts(:,1)',pts(:,2)');
    obstacles(thing) = 1; % mark the obstacle nodes in logical grid
end
    ...
    for j = 1:-1:-1
        if (k ~= j || k ~= 0)
            ...
            if((tempx > 0 && tempx <= MAX_X)) && ((tempy > 0 && tempy <= MAX_Y))
                flag = 1;
                for c1 = 1:c2 % if it's in closed list, don't expand
